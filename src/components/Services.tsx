...
export default function Services() {
  const { t } = useLanguage();
  const [activeTab, setActiveTab] = useState<"medicare" | "insurance" | "dental">("medicare");
  ...
  return (
    <section id="services" className="py-20 lg:py-28 bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Dental Content */}
        {activeTab === "dental" && (
          <div id="dental" className="space-y-16 animate-fadeIn">
            {/* Dental Overview */}
            <div className="bg-white rounded-2xl p-8 lg:p-12 shadow-sm border border-gray-100">
              <h3 className="text-2xl font-bold text-[#1B2A4A] mb-4">
                {t.dental.headline}
            ...
            <div>
              <div className="grid md:grid-cols-3 gap-6">
                {(Object.entries(t.dental.services) as [string, { title: string; description: string }][]).map(([key, service]) => (
                  <div
                    key={key}
                    className="bg-white rounded-2xl p-6 lg:p-8 shadow-sm border border-gray-100 hover:shadow-md hover:border-[#1B2A4A]/20 transition-all group"
                  >
            ...
            {/* Dental CTA */}
            <div className="bg-[#1B2A4A] rounded-2xl p-8 lg:p-12 text-center text-white">
              <h3 className="text-2xl font-bold mb-4">
                {t.dental.headline}
              </h3>
              <p className="text-white/80 leading-relaxed text-lg max-w-2xl mx-auto mb-8">
                {t.dental.subheadline}
              </p>
              <a
                href={CALENDAR_LINK}
                ...
                className="inline-flex items-center bg-white text-[#1B2A4A] px-8 py-4 rounded-full text-lg font-semibold hover:bg-gray-100 transition-colors shadow-lg"
              >
                {t.dental.cta}
                <svg className="w-5 h-5 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
